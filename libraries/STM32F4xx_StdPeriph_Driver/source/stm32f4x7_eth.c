...
ETH_DMADESCTypeDef  DMARxDscrTab[ETH_RXBUFNB];/* Array of Ethernet Rx DMA Descriptor */
#pragma data_alignment=4
ETH_DMADESCTypeDef  DMATxDscrTab[ETH_TXBUFNB];/* Ethernet Tx DMA Descriptor */

#pragma data_alignment=4
uint8_t aau8EthRxBuffs[ETH_RXBUFNB][ETH_RX_BUF_SIZE]; /* Ethernet Receive Buffers */
#pragma data_alignment=4
uint8_t aau8EthTxBuffs[ETH_TXBUFNB][ETH_TX_BUF_SIZE]; /* Ethernet Transmit Buffers */
...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  /* Drops frames with TCP/IP checksum errors */
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  /* Enable the Ethernet peripheral clock */
  RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_ETH_MAC, DISABLE);
...
//  if(!(ETH_WritePHYRegister(PHYAddress, PHY_BCR, PHY_Reset))) /* !!!!! */
  if(!(ETH_WritePHYRegister(REG_RESET, PHY_Reset))) /* !!!!! */
  {
    /* Return ERROR in case of write u32Timeout */
    return ETH_ERROR;
  }
  ...
  /* SCI: We are hardwired to port 3 of the Micrel switch, so can guarantee that the settings will match
  as long as we don't screw up the configuration.  Auto-negotiation is not required, then, so we
  skip detection and hard-code 100M full duplex. */

  ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
  ETH_InitStruct->ETH_Speed = ETH_Speed_100M;

#if 0
  /* There is no auto-negotiation or link status on Port 3 of the switch -- only port 1 and port 2.
  For now, assume Port 1 is the upstream link so only monitor it.  Port 1 is connector J7 which is
  the jack closest to the LCD. */
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    {
      u32Timeout++;
//    } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (u32Timeout < PHY_READ_TO)); /* !!!!! */
    } while (!(EthSmiReadRegister(REG_PORT1_STATUS0) & _PHY_LINK_GOOD) && (u32Timeout < PHY_READ_TO)); /* !!!!! */

    /* Return ERROR in case of u32Timeout */
    if(u32Timeout == PHY_READ_TO)
    {
      return ETH_ERROR;
    }
    ...
    {
      u32Timeout++;
    } while (!(EthSmiReadRegister(REG_PORT1_STATUS0) & _PHY_AUTO_NEGO_COMPLETE) && (u32Timeout < PHY_READ_TO)); /* !!!!! */
//    } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (u32Timeout < (uint32_t)PHY_READ_TO));
    ...
    u8RegValue = EthSmiReadRegister(REG_PORT1_STATUS0);

    /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
    ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
    ETH_InitStruct->ETH_Mode  = ETH_Mode_FullDuplex;

    if(u8RegValue & _PHY_HALF_DUPLEX100_STATUS)
    {
      ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;
    }
    else if(u8RegValue & _PHY_FULL_DUPLEX10_STATUS)
    {
      ETH_InitStruct->ETH_Speed = ETH_Speed_10M;
    }
    else if(u8RegValue & _PHY_HALF_DUPLEX10_STATUS)
    {
      ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;
      ETH_InitStruct->ETH_Speed = ETH_Speed_10M;
    }
...
#if 0  /* Auto-negotiation not enabled: leave as defaults in switch */
  else
  {
...
void ETH_DMARxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff, uint32_t RxBuffCount)
{
  ...
  for(i=0; i < RxBuffCount; i++)
  {
    ...
    else
    {
      ...
      DMARxDesc->ControlBufferSize |= ETH_DMARxDesc_RER; /* Receive End of Ring */
    }
  }
...
void ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBuffCount)
{
  ...
  for(i=0; i < TxBuffCount; i++)
  {
    ...
    else
    {
      ...
      DMATxDesc->Status |= ETH_DMATxDesc_TER; /* Transmit End of Ring */
    }
  }
...
/**
  ...
  *     @arg ETH_DMA_FLAG_RWT : Receive watchdog u32Timeout flag
  ...
  *     @arg ETH_DMA_FLAG_TJT : Transmit jabber u32Timeout flag
...
/**
  ...
  *     @arg ETH_DMA_IT_RWT : Receive watchdog u32Timeout interrupt
  ...
  *     @arg ETH_DMA_IT_TJT : Transmit jabber u32Timeout interrupt
...
/**
  ...
  *     @arg ETH_DMA_IT_RPS : Receive process stopped interrupt
  *     @arg ETH_DMA_IT_RBU : Receive buffer unavailable interrupt
  *     @arg ETH_DMA_IT_R   : Receive status interrupt
  *     @arg ETH_DMA_IT_TU  : Underflow interrupt
  *     @arg ETH_DMA_IT_RO  : Overflow interrupt
  *     @arg ETH_DMA_IT_TJT : Transmit jabber u32Timeout interrupt
  *     @arg ETH_DMA_IT_TBU : Transmit buffer unavailable interrupt
...
/*                                PHY functions                               */
/******************************************************************************/
#if 0 /* Replaced in ethernet_peripheral_driver.c for switch-specific implementation */
/**
  ...
  * @retval ETH_ERROR: in case of u32Timeout
...
__IO uint32_t u32Timeout = 0;
  ...
  do
  {
    u32Timeout++;
    tmpreg = ETH->MACMIIAR;
  } while ((tmpreg & ETH_MACMIIAR_MB) && (u32Timeout < (uint32_t)PHY_READ_TO));
  /* Return ERROR in case of u32Timeout */
  if(u32Timeout == PHY_READ_TO)
  {
...
uint32_t ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg, uint16_t PHYValue)
{
  ...
  do
  {
    u32Timeout++;
    tmpreg = ETH->MACMIIAR;
  } while ((tmpreg & ETH_MACMIIAR_MB) && (u32Timeout < (uint32_t)PHY_WRITE_TO));
  /* Return ERROR in case of u32Timeout */
  if(u32Timeout == PHY_WRITE_TO)
  {
    return ETH_ERROR;
  }
...
#if 0 /* Port 3 on the switch does not have a loop back mode.  Comment this out for
now though maybe it will be useful to set Port 1 or Port 2 to loop back? */
/**
